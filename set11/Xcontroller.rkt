      ...
      (cond
        ...
        [ball-selected?
         (local
           ...
           (if (inside-wall-relative? (viewX new-px) (viewY bdy))
               (send model execute-command
                 (make-set-position new-px bdy))
               1257))]
...
(require "Model.rkt")
(define m (new Model%))
(define xy1 (new XController% [width 100] [height 50] [model m]))
...
(define (test:step1)
  (begin    
    ...
    (send xy1 after-button-down 255 225)
    (send xy1 after-drag 260 230)
    (send xy1 after-button-up 260 225)
